...
export default function Estoque() {
  ...
  function Form({ initial = {}, onCancel, onSave }) {
    ...
    function change(e){ const { name, value } = e.target; setF(prev => ({...prev, [name]: value})); }
    ...
    return (
      <div className="modal">
        <form className="modal-card" onSubmit={submit}>
          ...
          <label>Produto<input name="produto" value={f.produto || ""} onChange={change} required /></label>
          <label>Quantidade<input name="quantidade" value={f.quantidade || ""} onChange={change} /></label>
    ...
    <div>
      ...
      <div className="content-container">
        <div className="top-actions">
          <button onClick={() => { setEditing(null); setShowForm(true); }}>Adicionar item</button>
        </div>

        {loading ? <p>Carregando...</p> : (
          <table className="tabela">
            <thead><tr><th>Produto</th><th>Quantidade</th><th>Ações</th></tr></thead>
            <tbody>
              {items.length === 0 && <tr><td colSpan="3">Nenhum item no estoque.</td></tr>}
              {items.map(it => (
